...
import { Map, ShieldAlert, CheckCircle2, XCircle, Clock, Search, BookOpen } from 'lucide-react';
import Link from 'next/link';

export default function CadastreDashboard() {
  ...
  const handleAction = async (id: string, newStatus: 'Validé techniquement' | 'Falsifié' | 'Litige') => {
    setActionLoading(id);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-brand-surface border border-slate-200 dark:border-brand-border rounded-3xl p-8 shadow-xl">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                Dashboard <span className="text-brand-primary">Cadastral</span>
              </h1>
            </div>
            <p className="text-slate-500 dark:text-slate-400 font-medium">
              Vérification technique et spatiale des dossiers numérisés par les agents (Niveau 2).
            </p>
          </div>
          <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-6 py-4 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-widest">En attente (Cadastre)</p>
        ...
        <div className="w-full bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30 rounded-2xl p-5 flex items-start gap-4">
          ...
          <div>
            <h3 className="text-blue-600 dark:text-blue-400 font-bold text-sm uppercase tracking-wide">Instructions pour le Chef du Cadastre</h3>
            <ul className="text-sm text-blue-700/80 dark:text-blue-300/80 mt-2 space-y-1 list-disc list-inside">
              <li>Vérifiez l'exactitude des informations techniques et géographiques (Superficie, Mesurage).</li>
              <li>Assurez-vous qu'il n'y a <strong>aucune superposition</strong> spatiale avec des parcelles voisines.</li>
              <li>Si le dossier est erroné techniquement, rejetez-le.</li>
              <li>Si le dossier est propre, cliquez sur "Valider Techniquement" pour l'envoyer au <strong>Conservateur (Niveau 3)</strong>.</li>
            </ul>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white dark:bg-brand-surface border border-slate-200 dark:border-brand-border rounded-3xl">
            ...
            <p className="text-slate-500">Toutes les vérifications topographiques sont à jour.</p>
          </div>
        ) : (
                  ...
                  {titles.map((title) => (
                    <tr key={title.id} className="hover:bg-slate-50 dark:hover:bg-brand-bg/50 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        ...
                        <p className="text-xs text-slate-500">Superficie: {title.superficie || 'Non définie'}</p>
                      </td>
                      <td className="py-4 px-6">
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            title="Rejeter (Erreur Technique)"
                          >
                            <XCircle className="w-6 h-6" />
                          </button>
                          <button
                            onClick={() => handleAction(title.id, 'Validé techniquement')}
                            disabled={actionLoading === title.id}
                          ...
                          >
                            ...
                            {actionLoading === title.id ? '...' : 'Valider Techniquement'}
                          </button>
                        </div>
                      </td>
                    </tr>
